...
# Use dgps coordinates from '_comnav_dgps.dat' files. Set to 'False' to use GPS from 'maretron' files
dgps = True
...
for file in os.listdir(data_path):
    if '_maretron_nmea_gateway.dat' in file:
        ...
        with open(full_path, 'r') as f:
            for line in f:
                ...
                elif '$IIGLL' in line and not dgps:
                    parts = line.strip().split(',')
    ...
    if '_comnav_dgps.dat' in file and dgps:
        files_cnt += 1
        print(f'Processing file: {file}')
        full_path = os.path.join(data_path, file)
        ...
        with open(full_path, 'r') as f:
            for line in f:
                if '$GPGGA' in line and dgps:
                    parts = line.strip().split(',')
                    if len(parts) >= 7 and parts[3] and parts[4] and parts[5] and parts[6]:
                        lat_dd, lon_dd = convert_to_dd(parts[3], parts[4]), convert_to_dd(parts[5], parts[6])
                        gps_rows.append([parts[0], parts[3], parts[4], parts[5], parts[6], lat_dd, lon_dd])
...
###
#df_gps.to_csv('gps_only.csv', index=False)
###
