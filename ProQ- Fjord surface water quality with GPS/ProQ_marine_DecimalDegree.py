...
# Define a lambda function to convert DDM to DD and handle North/South
# def convert_to_dd(ddm_str, nswe_str):
#     degrees = int(float(ddm_str)/100)
#     minutes = float(ddm_str) - degrees * 100
#     ddm = degrees + minutes/60
#     if nswe_str in ['S', 'W']:
#         ddm *= -1  # Negate latitude if South
#     return round(ddm, 5)
...
if gps_rows:
    ...
    df_gps['Latitude_DD'] = df_gps.apply(lambda row: convert_to_dd(row['Latitude'], row['NS']), axis=1)
    df_gps['Longitude_DD'] = df_gps.apply(lambda row: convert_to_dd(row['Longitude'], row['WE']), axis=1)
